"""Test cases for SymConf parameter traversal (遍歷參數值).
...
class TestParameterTraversal:
    """Test parameter traversal functionality (遍歷參數值)."""

    def test_manual_traversal(self, temp_dir: Path):
        ...
        for dataset in ["iris", "cifar10"]:
            for batch_size in [32, 64]:
                ...
                config = parser.parse_args(
                    [str(config_path), "--args", f"dataset={dataset}", f"batch_size={batch_size}"]
                )
    ...
    def test_simple_sweeping(self, temp_dir: Path):

...
- Given: 初始化 `TwinConf` parser 並解析命令列參數
    ```python
    # main.py
...
以下的每個檢查在 `TwinConfParser` 初始化時都可以選擇性關閉，但預設為開啟
...
範例：
- Given: 初始化 `TwinConfParser` 時指定了 key 的 base_classes
    ```python
    parser = TwinConfParser(
        base_classes={
            'model': Parent,
...
為了避免複雜的設置建構過程中出錯，TwinConf 提供 `--print` 來印出最終建構的設置，並要求使用者按 Enter 確認後才會執行
...
- `**kwargs` 追蹤
    - 當物件參數有 `**kwargs`，且在函式內部以 `**kwargs` 展開作為其他物件的參數時，TwinConf 能辨認出其他物件的參數也屬於該物件的參數。
